...
def validate_module_layout(task: TaskSpec, module_dir: Path | str) -> None:
    ...
    logging.debug(f"[Bootstrap] Copied .cfg config from template at {cfg_target}")
    ...
    with open(tla_target, "w", encoding="utf-8") as f:
        f.write(content)
    logging.debug(f"[Bootstrap] Copied TLA+ module from template at {tla_target}")
...
def purge_temp_modules(task_name: str, module_root: Path | str) -> PurgeStats:
    ...
    if removed:
        logging.debug(f"[ModuleCleanup] {stats.as_log_message()}")
    return stats
...
def generate_cfg_via_llm(module_snapshot, task, provider, attempt_id, prompts_dir, cfg_generation_template=None):
    ...
    logging.debug(f"[LLM-CFG] Wrote LLM-generated TLC config to {out_cfg_path}")
