...
def build_mermaid_prompt(tlc_trace: str) -> str:
    return (
        ...
        "- For Consent legal basis bars, the start time MUST be the time of the corresponding GiveConsent event, and the end time MUST be the time of the corresponding WithdrawConsent event (if present). If the WithdrawConsent event is missing, use the final state's now time as the end time. Do NOT use any other time points for Consent bars.\n"
        "- For DataBreach bars, the start time MUST be the time of the corresponding DataBreachDetected event, and the end time MUST be the time of the corresponding DataBreachReported event (if present). If the DataBreachReported event is missing, use the final state's now time as the end time. Do NOT use any other time points for DataBreach bars.\n"
