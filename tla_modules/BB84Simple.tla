...
Transmit ==
    /\ \E b \in Bases :
         channelState' \in StatesOfBasis(b)
    /\ UNCHANGED <<aliceState, bobBasis, bobResult>>
    
Measure ==
    /\ IF Basis(channelState) = bobBasis
...
IdealCorrectness ==
    Basis(aliceState) = bobBasis
    => bobResult = BitValue(aliceState)

PerfectChannelCorrectness ==
    ChannelIntegrity => IdealCorrectness

KeyTypeSafety ==
    /\ AliceKeyBit \in Bits \cup {"discard"}
    /\ BobKeyBit \in Bits \cup {"discard"}
